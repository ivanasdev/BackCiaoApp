...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data->email) && isset($data->KeyNV)) {
        // Sanitizar y asignar valores
        $email = $conn->real_escape_string($data->email);
        $KeyNV = $conn->real_escape_string($data->KeyNV);

        // Consulta SQL para el login
        $queryAppLogin = "SELECT * FROM tbl_NutritionistRequest WHERE st_Email = '$email' AND st_Password = '$KeyNV'";
